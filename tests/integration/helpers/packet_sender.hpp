...
public:
    ...
    bool send_arp_request(const std::string& src_mac, const std::string& src_ip,
        ...
        // src MAC
        uint8_t mac[6];
        sscanf(src_mac.c_str(), "%hhx:%hhx:%hhx:%hhx:%hhx:%hhx", &mac[0], &mac[1], &mac[2], &mac[3],
               &mac[4], &mac[5]);
        ...
        ssize_t sent =
            sendto(sockfd, packet.data(), packet.size(), 0, (struct sockaddr*) &addr, sizeof(addr));
        ...
        std::cout << "Sent ARP request: " << src_ip << " -> " << dst_ip << " (" << sent << " bytes)"
                  << std::endl;
        return true;
    }
    ...
    bool send_icmp_ping(const std::string& src_mac, const std::string& dst_mac,
        ...
        sscanf(dst_mac.c_str(), "%hhx:%hhx:%hhx:%hhx:%hhx:%hhx", &dmac[0], &dmac[1], &dmac[2],
               &dmac[3], &dmac[4], &dmac[5]);
        sscanf(src_mac.c_str(), "%hhx:%hhx:%hhx:%hhx:%hhx:%hhx", &smac[0], &smac[1], &smac[2],
        ...
        std::cout << "Sent ICMP ping: " << src_ip << " -> " << dst_ip << " (" << sent << " bytes)"
                  << std::endl;
        return true;
    }
